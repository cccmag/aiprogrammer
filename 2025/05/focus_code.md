...
本篇文章展示一個完整的 Web API 應用範例，內容涵蓋 requests 呼叫外部 API、JSON 資料處理、FastAPI 應用建置，以及 API 認證機制。所有程式碼可在 `_code/` 目錄中找到。

---

## 原始碼

完整的 Python 實作請參考：[_code/web_api.py](_code/web_api.py)
...
序列化輸出:
{
  ...
  "skills": ["Python", "API"],
  "active": true,
  "registered": "2026-05-15T10:30:00"
}
反序列化驗證通過

=== FastAPI 模擬範例 ===
建立物品:
  {'status': 201, 'data': {'id': 1, 'name': '筆記本', 'price': 29.99}}
  {'status': 201, 'data': {'id': 2, 'name': '原子筆', 'price': 9.99}}
...
所有範例執行完畢！
```
...
**demo_requests()**：使用 `requests` 套件呼叫 JSONPlaceholder 公開 API，取得使用者貼文資料。展示 GET 請求、參數傳遞、回應解析等基本操作。

**demo_json()**：示範 Python 物件的 JSON 序列化與反序列化。包含中文字元處理、格式化和型別轉換。

**demo_fastapi_simulation()**：不使用 FastAPI 框架，用純 Python 模擬 RESTful API 的 CRUD 操作。展示路由設計、輸入驗證、錯誤處理等概念。

**demo_auth()**：示範 API Key 生成與驗證，以及簡化的 JWT Token 產生。

**demo_rate_limit()**：實作簡單的滑動視窗速率限制演算法。
...
- [FastAPI 官方文件](https://www.google.com/search?q=FastAPI+documentation)
- [JSON 官方網站](https://www.google.com/search?q=JSON+official)

---

*本篇文章為「AI 程式人雜誌 2026 年 5 月號」焦點主題補充文章。*
