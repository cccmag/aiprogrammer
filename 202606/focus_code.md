...
理論說得再多，不如親手實作一個 SQL 資料庫引擎。本篇文章將帶領讀者從零開始，用 Python 實作一個名為「MiniSQL」的小型記憶體式關聯式資料庫。

MiniSQL 支援：
- 資料表建立（CREATE TABLE）
- 資料插入（INSERT INTO）
- 查詢（SELECT）支援 WHERE 條件與 AND/OR 邏輯
- 資料刪除（DELETE）
- 資料表刪除（DROP TABLE）
- 互動式 REPL 模式
...
完整的 Python 實作請參考：[_code/minisql.py](_code/minisql.py)
...
class Database:
    ...
    def _delete(self, stmt):
        ...
        if stmt.where:
            ...
            table.rows = []
        return f'OK: {before - len(table.rows)} rows deleted from "{stmt.table_name}"'
...
OK: Table "employee" created

SQL: INSERT INTO employee VALUES (1, 'Alice', 75000);
OK: 1 row inserted into "employee"

SQL: INSERT INTO employee VALUES (2, 'Bob', 68000);
...
...
SQL: SELECT name, salary FROM employee WHERE salary > 75000;
+---------+--------+
| name    | salary |
+---------+--------+
| Charlie | 82000  |
...
SQL: SELECT name FROM employee WHERE salary >= 72000 AND salary <= 82000;
+---------+
| name    |
+---------+
| Alice   |
...
SQL: DELETE FROM employee WHERE name = 'Eve';
OK: 1 rows deleted from "employee"

SQL: DROP TABLE employee;
OK: Table "employee" dropped
```
...
```
$ python minisql.py repl
MiniSQL v1.0
Enter SQL statements (end with ;). Type 'exit' to quit.
sql> CREATE TABLE products (id INT, name TEXT, price FLOAT);
sql> INSERT INTO products VALUES (1, 'Laptop', 39900.0);
sql> INSERT INTO products VALUES (2, 'Mouse', 1290.0);
sql> INSERT INTO products VALUES (3, 'Keyboard', 3990.0);
...
SQL 查詢字串
    ...
    ▼
┌──────────────┐
│  詞法分析器   │  SQL 字串 → Token 串流
│  (Lexer)     │
└──────┬───────┘
       ▼
┌──────────────┐
│  語法分析器   │  Token → 抽象語法樹 (AST)
│  (Parser)    │
└──────┬───────┘
       ▼
┌──────────────┐
...
1. **加入 B-tree 索引**：實作簡單的 B-tree 結構來加速查詢
2. **支援 ORDER BY**：加入排序功能
3. **支援 JOIN**：實作兩表 JOIN 查詢
4. **交易支援**：實作 BEGIN/COMMIT/ROLLBACK
5. **磁碟持久化**：將資料寫入檔案以實現持久儲存
6. **查詢最佳化**：加入簡單的查詢最佳化（如 WHERE 條件的順序調整）

---

## 結語

MiniSQL 雖然只有不到 400 行 Python 程式碼，但它實現了一個資料庫引擎的核心功能：
...
這個實作展示了資料庫系統的設計模式——當你理解了詞法分析、語法分析和執行引擎這三個核心概念，你就可以在此基礎上建構任意複雜的資料庫應用。

詳細的技術背景請參考：
- [關聯式資料庫的誕生](focus2.md) — 關聯式模型與 SQL 的歷史
- [儲存引擎與索引技術](focus3.md) — 高效資料存取的核心
- [SQL 查詢處理與最佳化](focus4.md) — 查詢引擎的設計原理
